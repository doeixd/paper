...
param(
    [string]$InputFile = (Join-Path (Split-Path -Parent $PSScriptRoot) "Computational-Closure-and-the-Architecture-of-Mind.md"),
    [switch]$DryRun
)
...
$content = Get-Content $resolvedInput -Raw
...
# Save the file
if ($DryRun) {
    Write-Host "Dry run complete. No changes were written to $resolvedInput"
} else {
    $content | Set-Content $resolvedInput -NoNewline
    Write-Host "Transformation complete for $resolvedInput"
}
