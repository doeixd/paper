...
A "Shoe-Moon" aggregate fails the $\varepsilon$-machine test because treating them as a unit provides zero information compression. The joint description requires exactly as much information as describing the parts separately; there is no predictive shortcut. This can be formalized via Kolmogorov complexity: genuine wholes allow compressed descriptions where the code describing the system's evolution as a unit is shorter than the code describing its parts independently. Trivial dimensions are **brittle**; they require constant ad-hoc parameters to maintain prediction. Genuine dimensions are **low-maintenance** because they compress the state space. Real entities earn their ontological status by reducing the computational cost of prediction. If a boundary does not achieve compression, it does not exist.
