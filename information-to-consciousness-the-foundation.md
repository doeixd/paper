...
Living and cognitive systems differ fundamentally. They compress information, building internal models that predict future sensory states. This compression is literal (Shannon's sense), not metaphorical: reducing surprise by encoding regularities into reusable patterns.
...
The Strong Pivot—Embracing the Tautology: This critique deserves a direct response, and we can offer one that strengthens rather than weakens our philosophical framework. Even if Mangalam is correct that the FEP is tautological—that it describes necessary consequences of being a bounded system rather than contingent empirical laws—this would make it precisely the right foundation for our philosophical project.

Consider the analogy to logic itself. Logic is tautological: the law of non-contradiction and excluded middle follow necessarily from the structure of coherent reasoning. You cannot empirically falsify them because they describe the preconditions for meaningful empirical inquiry. Yet this tautological status does not make logic "pseudo-theory" or "vacuous." Rather, logic provides the necessary grammar for any system capable of error correction and belief revision.

The FEP occupies the same structural role. If it describes necessary constraints that any bounded system must satisfy to persist against entropy—that maintaining a statistical boundary requires minimizing the divergence between internal model and environmental structure—then it functions as the grammar of viable self-organization. It tells us what any system must do to exist as a "thing" with persistence through time.
...
Phenomenological Correspondences:
...
The Self as a User Interface: Following Rosas et al. (2024), the "Self" is not a ghost in the machine but a user interface. It represents the brain's own lossy compression of its massive, distributed neural activity.

Just as a computer operating system represents billions of transistor states as a single "folder" icon, the brain compresses its complex somatic and cognitive states into a single variable: "I".
...
Any system that learns (compresses experience, updates on prediction error) must implement logical structure. Logic is not selected by systems; it is the operating system of selection itself.
...
This dual status grounds realism (there is an objective structure) while preserving fallibilism (we cannot claim certainty about fully capturing it).
...
What we can legitimately claim: Certain moral intuitions have information-theoretic grounding. The recognition that denying others' agency generates catastrophic social costs helps explain why such behaviors are unsustainable. It also helps explain why moral progress often tracks toward lower-brittleness configurations.
