...
microphysical furniture \citep{ladyman2007}. But the central question
remains underdetermined. Which patterns are admitted into that
rainforest, and which are merely useful summaries, temporary proxies,
...
not change macro-level what-follows. Where screening off shows that
micro-details are conditionally irrelevant for a target dependence,
closure shows that the candidate carries its own transition structure
under admissible perturbation.

The narrative is cumulative rather than competitive. Dennett's real
...
This thesis is a completion move, not a replacement project. Existing
...
parasitic survivors by transition structure rather than by intuitive
naturalness. The formal benchmark for this condition is strong
...
The current real-patterns literature makes this timing unusually good.
Millhouse's reconstruction of ``really real'' patterns tightens
Dennett's realism without yet requiring transition autonomy
\citep{millhouse2022}. Wallace's recent structural treatment shifts the
...
and mechanisms shows that the demand for a sharper macro-ontological
criterion is now spreading across domains
\citep{jiang2024,jiang2025}. The first dedicated volume on real
patterns confirms that this is no longer a marginal discussion, and
Ladyman's contribution there presses in a direction close to the
present paper's dynamic emphasis \citep{millhouseetal2026,ladyman2024}.
...
that sense, intervention operationalizes effective realism rather than
competing with it.
...
admittance filter, and the case just described is not meant to trivialize
it. The point is narrower. Under passive observation, the common-cause
proxy can screen off micro-details relative to the relevant
macro-explanandum, and it can support a non-trivial macrodependency that
looks novel at that level. In that sense, it can survive the best
current screening-off filter. Yet its apparent autonomy is still
borrowed from the upstream driver. Once admissible intervention
separates the proxy from that shared cause, the transition structure
collapses. Screening off was real, but parasitic.
...
screening off plus novelty, what substantive work is left for closure to
do \citep{franklinrobertson2024}?
...
Under passive observation, a proxy variable can screen off micro-detail
and support a stable macrodependency because an upstream driver keeps
its trajectories synchronized. In that sense, the screening-off result
is genuine. But the variable remains parasitic on the shared source. If
an admissible intervention perturbs the proxy while leaving the driver
...
sorts one class of survivors that screening off alone cannot exclude.

This is not a hostile revision of Franklin and Robertson's program. On
the contrary, the present paper relies on it. Their criterion removes
obvious gerrymanders and clarifies the importance of law-like
macrodependence. The present claim is simply that one more filter is
needed for robust admission to the rainforest. Compression identifies
candidates. Screening off plus novelty narrows the field. Closure then
determines which survivors remain autonomous under admissible
perturbation. That is a completion of the admittance protocol, not a
replacement.
