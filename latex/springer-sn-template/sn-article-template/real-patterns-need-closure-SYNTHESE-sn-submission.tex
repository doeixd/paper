...
some data, and is real, if there is a description of the data that is more
...
genuinely general and compressive while still grouping
...
bookkeeping to maintain forecast quality, because the
macro-label hides transition-relevant heterogeneity. This residual
permissiveness survives Dennett's own anti-cheating move,
...
\citep{haugeland1998}, and later work on idealization continues to
press the gap between descriptive utility and epistemic warrant
\citep[chap.~2]{elgin2017}. The present paper
...
and hierarchical. Interventions are the empirical case of a broader
admissible-transformation idea: what matters is fixing a defensible class
of allowed manipulations before closure is scored.

The admissible intervention class determines which possible disturbances
a candidate macro-object must survive, and that range of possibilities
is what gives the regime its evaluative content. Objecthood, on this
view, is stability of macro-transition structure across the space of
admissible perturbations, not merely fit to observed trajectories. The
constraints below ensure that this space is physically anchored rather
than freely stipulated.
...
A critical clarification prevents this admissibility requirement from
collapsing into anthropocentrism. While we often speak of
``interventions'' as laboratory actions performed by human scientists,
admissibility is ultimately anchored in the objective physical ecology
...
whether a human experimenter is observing it or not. Admissibility
tracks these naturally occurring physical boundary conditions and energy
scales. The intervention class is therefore not a human invention; it is
a formalization of the physical perturbations the target system already
reliably processes or resists.
...
Before moving to the formal benchmark, two operational consequences
follow from the criterion stated in
\S\ref{canonical-criterion-statement}.
...
require perfect closure everywhere. The study of
approximate aggregation has a long history, including analyses of
nearly decomposable systems that separate short-run
within-subsystem dynamics from long-run aggregate behavior
\citep[pp.~111--117]{ando1961}. The present extension is
...
channels have shifted. Since the regime is partly defined by its
admissible intervention class
(\S\ref{admissibility-and-hierarchical-evaluation}), a change in which
perturbations are available can alter the closure verdict even when the
...
Reply: that is correct, and the account does not deny it. Prediction
...
Clear limits remain. The proposal is not a complete metaphysics of
