...
passing screening-off style tests while failing under admissible intervention
(\S\ref{what-the-formal-machinery-alone-does-not-provide}).

Three nearby traditions further sharpen the paper's posture. Simon's
...
treatment of closure defended here. But Simon asks when aggregation is
dynamically serviceable, not when such serviceability warrants
ontological commitment.

Wimsatt's analysis of aggregativity marks a second antecedent. He argues
that genuinely aggregative system properties are exceptional because most
...
carries self-contained transition structure. Closure adds that further
test.
...
precise. Millhouse tightens Dennett's realism without yet requiring
transition autonomy \citep{millhouse2022}. He is the clearest nearby
case of a stronger real-pattern view that still stops short of the
transition test defended here, because predictive success and
high-level mapping can remain too permissive without a stricter
dynamical condition \citep{millhouse2022}. Wallace presses the
discussion toward patterns in dynamics and distributions rather than
mere collections of parts \citep{wallace2024}. Jiang's work in biology
and mechanisms shows that closely related ontological questions are now
being asked across multiple domains \citep{jiang2024,jiang2025}. In
mechanisms, Jiang explicitly uses structural realism to press against
object-based metaphysics; in biology, the same broad terrain is framed
more permissively in terms of explanatory and pragmatic utility
\citep{jiang2024,jiang2025}. The
recent volume on real patterns, together with Ladyman's contribution to
it, confirms that the dynamic and structural interpretation of pattern
realism is now an explicit topic rather than a background implication
\citep{millhouseetal2026,ladyman2026}. The present paper's narrower
claim is that dynamic or phase-space formulations still leave open an
admittance and downgrade question. Closure is proposed as that further
discipline, not as a rival to the move toward dynamic structure.
...
projects, but whether onward macro-transitions are self-contained at the
declared grain. That is the gap this paper claims remains even after the
best current screening-off proposals.

In short, projectibility and screening off can still tolerate
successful forecasting that depends on recurring micro-level repair.
Closure does not.
...
present proposal does not offer a full mereology. It does, however,
offer a positive answer in the intended register: robust transition
autonomy under declared constraints is sufficient for composition in
regime, even if it does not settle every composition question outside
that regime. Composition claims about macro-objects are therefore
warranted when transition autonomy is robust under declared constraints,
and should be withheld when candidate sums remain transition-fragile.
...
answer is not a loose plurality of levels but the ordered space of
computationally closed coarse-grainings. Rosas et al. provide the formal
architecture for talking about such relations among partitions
\citep{rosas2024}. The present paper's claim is philosophical: lattice
membership is what it is for a candidate level to become ontologically
serious. Admission, qualification, and downgrade are therefore not
floating labels imposed from outside the formalism. They locate a
candidate within a structured space of more and less stable closure
relations.
...
The proposal is accordingly not vulnerable to standard ``bare
...
The same point clarifies the relation to mechanistic views of levels.
Mechanistic accounts are right that organized entities and activities can
support stable explanation and intervention \citep{machamer2000,craver2007}. But by themselves they do not settle whether a proposed
...
present account rejects that deflation because closure claims are indexed to
...
criterion is designed to preserve. The modal profile, not the in-sample
...
The view therefore distinguishes epistemic humility from
...
tracks ontological seriousness is the present paper's philosophical
contribution.
...
Those verdicts are also action-guiding for philosophical practice.
Robust verdicts license unqualified ontology talk in the declared regime
and explanatory reliance without special caveats. Qualified verdicts
license selective commitment only with explicit regime, horizon, or
admissibility limitations, and they should prompt active comparison with
nearby candidate partitions. Indeterminate verdicts do not license
settled ontology claims at all. They call for suspension, partition
revision, or further evidential work.
...
macrodependency from an upstream driver and can thereby make a range of
micro-detail conditionally irrelevant relative to the target
explanandum. More exactly, the case can satisfy the package Franklin and
Robertson identify. The proxy is unconditionally relevant because its
states track target-relevant differences. It is conditionally irrelevant
relative to the target because, once the proxy state is fixed, a range
of lower-level differences in the driven subsystems add no further
explanatory leverage in the observational regime. And it contributes the
kind of macro-level dependence that makes the target explanandum look
serious at that grain. That is why the case is a genuine pressure case
for the screening-off package rather than a merely verbal
counterexample. The point is not that screening off was misdiagnosed and
later exposed as fictitious. The point is that screening off is
observationally genuine relative to the target explanandum while still
remaining insufficient for robust macro-objecthood.

Closure asks the further question Franklin and Robertson do not
themselves need to ask: does the proxy partition preserve its own
transition structure when it is directly and admissibly perturbed? In
the common-cause case, the answer is no. The screening off was not
imaginary, but parasitic in the precise sense that its success depended
on upstream structure the proxy could not itself stabilize under direct
admissible perturbation. Closure therefore sorts one class of survivors
that screening off alone does not yet exclude.
