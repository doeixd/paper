...
relaxes it toward its apparent attractor. The source of the borrowed
stability is now internal and fast rather than upstream and exogenous,
but the skeleton of the previous case transfers directly, which is the
point worth establishing: the gap is not tied to one configuration.
...
The two cases differ in the source of the borrowed stability rather
than in the shape of the failure. The common-cause proxy borrows from a
shared upstream driver that is exogenous and persistent; hidden-feedback
parasitism borrows from an internal relaxation that is fast and
endogenous. That they share a formal skeleton is not a defect of the
presentation but the substance of the claim. What generates the gap in
both is a single structural fact, that the observed process visits only
a homogeneous subset of a heterogeneous macroclass, and the two sources
show that this fact does not depend on where the off-partition structure
happens to sit. A defender of the screening-off-plus-novelty package
might concede the common-cause proxy as an edge phenomenon while keeping
