...
pattern realism are now explicit options \citep{millhouse2022,wallace2024,jiang2024,jiang2025,millhouseetal2026,ladyman2026}. The present paper's narrower claim is that even these stronger views leave open a final admittance question. Closure is proposed as that further discipline, not as a rival to the move toward dynamic structure.
...
is made explicit in practice. Rosas et al. provide the formal
architecture for talking about nested closure structure; the present
paper's contribution is to turn that architecture into an ontological
reporting discipline \citep{rosas2024}.
...
otherwise misread the proposal as reducing ontology to whichever metric
...
Franklin and Robertson identify. The proxy is unconditionally relevant
because its states track target-relevant differences; conditionally
irrelevant because, once the proxy state is fixed, further lower-level
differences add no further explanatory leverage for the target in the
observational regime; and novel because the dependence is captured at
that grain rather than by repeated micro-enumeration. Closure then asks
the further question Franklin and Robertson do not themselves need to
ask: does the proxy partition preserve its own transition structure when
it is directly and admissibly perturbed? In the common-cause case, the
answer is no. The screening off was observationally genuine, but
parasitic in the precise sense that its success depended on upstream
structure the proxy could not itself stabilize under direct admissible
perturbation.
...
within its macroclasses stops mattering to macro-what-follows under the
declared constraints. That is the sense in which closure gives
