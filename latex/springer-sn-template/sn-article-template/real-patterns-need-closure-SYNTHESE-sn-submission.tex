...
discipline, and shows how closure sharpens the non-redundancy demand
already implicit in the Primacy of Physics Constraint. Section 3 gives
the exact benchmark and draws out the lattice payoff of computationally
closed coarse-grainings. Section 4 extends the criterion to non-ideal
settings and explains the graded verdict structure. Section 5 addresses
the strongest objections. Section 6 summarizes gains and limits.
One later dialectical step deserves advance notice. The paper argues
that projectibility can be genuine yet still parasitic: a pattern can
support stable projection while borrowing that stability from upstream
structure rather than carrying its own transition organization.

\subsection{Three Nearby Positions}\label{three-nearby-positions}

Three nearby stances should be kept separate.
...
\begin{enumerate}
	...
	\item
	      ...
	      present account objecthood requires transition autonomy under
	      fixed regime, horizon, and admissible intervention class.
\end{enumerate}

The third stance preserves the explanatory strengths of the first while
avoiding its permissiveness, and it avoids reducing ontology to
convenience, which is the pressure on the second.

\subsection{Contribution Map}\label{contribution-map}

The contribution map is simple.

\begin{enumerate}
	\item
	      It tightens real-pattern realism by adding closure as a
	      discriminating objecthood condition.
	\item
	      It carries the same criterion from exact benchmark cases to
	      non-ideal cases without changing its content.
	\item
	      It adds explicit verdict and downgrade conditions so realism
	      claims remain answerable to failure.
	\item
	      It clarifies why multiple realizability is not enough by itself:
	      realizers must also preserve the same macro-transitions under the
	      declared constraints
	      (\S\ref{autonomy-pluralism-and-distinctiveness}).
\end{enumerate}

The central payoff is selective commitment: strong where transition
autonomy is robust, cautious where evidence is fragile or
regime-sensitive.
...
present paper adds is the ontological reading: the rainforest's
structure is the lattice of computationally closed coarse-grainings,
within which admission, qualification, and downgrade can be reported
explicitly.
