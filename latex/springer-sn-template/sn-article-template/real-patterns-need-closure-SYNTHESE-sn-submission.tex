...
The dialectical structure is cumulative. Dennett's real
...
\citep{franklinrobertson2024}. Closure adds one further step by distinguishing the surviving candidates that are
genuinely autonomous from those that remain parasitic on hidden
micro-identity or a shared upstream cause. The aim is to complete
rainforest realism by supplying the missing dynamical downgrade
condition.
...
The thesis is additive. Existing pattern realism captures compression and projectibility
insights. The present contribution adds a discriminating condition that
excludes parasitic survivors by transition structure. It does not rely
on intuitive naturalness. The formal benchmark for this condition is strong
...
coarse-grainings, not as a binary inventory of what is, or is not,
real. A later section makes the key dialectical point explicit: a
pattern can remain projectible and yet still be parasitic if its
apparent stability is borrowed and does not arise from its own
...
Once closure is added, the rainforest becomes a disciplined space of
...
partitions. Some partitions are finer, some are coarser, and some nest
cleanly inside others. The point of invoking the lattice is simple.
Disciplined rainforest realism has a structured space of admissible
levels, not a mere list of candidates.
...
Applied consistently, the assessment sequence is straightforward.
...
In the recent literature, its role is specific. Relative to Franklin
and Robertson, it supplies a transition test for parasitic
...
And relative to recent dynamic and phase-space formulations of pattern
realism, including Wallace and Ladyman's contribution to the recent
volume, it adds admission and downgrade discipline to a dynamic picture
of higher-level organization \citep{wallace2024,ladyman2026}.

The proposal supports selective commitment: robust commitment where
closure is stable, qualified commitment in borderline cases, and explicit
downgrade where verdicts depend on fragile admissibility or horizon
choices. The proposal is intentionally
