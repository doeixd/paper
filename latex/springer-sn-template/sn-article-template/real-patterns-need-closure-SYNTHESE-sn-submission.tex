...
\(0 < q < 1/2\). Let \(A\) and \(B\) be subsystems that track their
respective components without lag. The system's specified dynamical
ecology includes one control channel, a reset line on \(B\) carrying a
value \(C_t\), so that the subsystems' update rule is
\(A_t = U^1_t\) and
\[
	B_t =
	\begin{cases}
		C_t   & \text{when the reset channel is actuated,} \\
		U^2_t & \text{otherwise.}
	\end{cases}
\]
The micro-state is \(X_t = (U^1_t, U^2_t, A_t, B_t)\). The candidate
macro-variable is the agreement of the two subsystems,
\(Z_t = A_t \oplus B_t\), which takes the value \(0\) when they agree
and \(1\) when they disagree.

The reset channel is part of the system's specification rather than an
intervention imagined from outside it. In the observed process it is
simply never actuated, so the subsystems are pinned to their components,
\(A_t = U^1_t\) and \(B_t = U^2_t\), and \(Z_t = U^1_t \oplus U^2_t\).
That is the point of including it. The restriction of the observed
process to the synchronized manifold is a fact about which channels
happen to be exercised, not a fact about what the dynamics permit.
...
\begin{enumerate}
	...
	\item
	      ...
	      condition.
	\item
	      Novelty. The point is structural rather than numerical. \(Z\)'s
...
process never visits: those in which the subsystems agree while their
driver components do not. Suppose the system is in the observed state
\(U^1_t = 0\), \(U^2_t = 1\), \(A_t = 0\), \(B_t = 1\), so that
\(Z_t = 1\), and actuate the reset channel with \(C_t = 0\), leaving
both driver components untouched. The system now occupies a realization
of \(Z_t = 0\). From there, \(P(Z_{t+1} = 0) = 1 - r\), because the
components still disagree, whereas every observed realization of
\(Z_t = 0\) yields \(r\). Two micro-realizations of the same macrostate
have different macro-futures, and \(r \neq 1 - r\) for \(q \neq 1/2\).
...
\begin{enumerate}
	...
	\item
	      ...
	      directly while the drivers remain intact.
	\item
	...
	\item
	      ...
	      parasitic. It was borrowing stability from the drivers
...
skeleton of the previous case transfers directly, which is the point
worth establishing: the gap is not tied to one configuration.
...
than in the shape of the failure. The latent-driver proxy borrows from a
...
\begin{enumerate}
	...
	\item
	      ...
	      distributions in which \(X_t\) is set by an admissible
	      intervention ranging over the reachable realizations of the
	      macrostate. This is the criterion's quantity. Because no single
	      such distribution is privileged, it is defined as a supremum
	      over them,
	      \[
		      \Lambda_I(Z) \;=\; \sup_{\mu \in \mathcal{M}_I}
		      I_\mu\!\left(X_t; Z_{t+1} \mid Z_t\right),
	      \]
	      where \(\mathcal{M}_I\) is the set of distributions over
	      within-class realizations reachable by interventions in the
	      admissible class \(I\). Taking the supremum rather than an
	      average is what makes the quantity a test of the candidate
	      rather than of a sampling policy: a partition does not earn
	      robust standing by being homogeneous under some convenient
	      weighting of its realizations.
	      Equivalently, and often easier to estimate, closure can be
	      stated pairwise as the requirement that admissibly reachable
	      \(x, x'\) with \(g(x) = g(x') = z\) induce the same
	      macro-transition kernel, so that
	      \(\sup_{x,x'} D\!\left(P(Z_{t+1} \mid do(X_t{=}x)) \,\|\,
	      P(Z_{t+1} \mid do(X_t{=}x'))\right)\) vanishes for a suitable
	      divergence \(D\). Either form vanishes just in case
	      within-class micro-differences do not alter the macro-transition
	      profile, which is transition closure as stated in
	      \S\ref{formal-statement}. In the latent-driver proxy the
	      observational quantity is exactly zero while both admissible
	      forms are strictly positive, which is the separation the
	      criterion turns on.
\end{enumerate}
...
downgrade conditions). A realism criterion that specifies no withdrawal conditions is harder to
assess, since nothing it rules out is stated in advance.
...
A brief illustration makes this concrete. Suppose \(P_{lane}\), the
lane-segment partition of the commuter corridor of
\S\ref{regime-individuation-and-circularity}, is
...
comparative ranking against the rival \(P_{tod}\) not flip
and that absolute leakiness for the leading candidate remain below the
declared viability floor across the extended horizons. If
\(P_{lane}\) dominates at $L = 15$ but the advantage vanishes
