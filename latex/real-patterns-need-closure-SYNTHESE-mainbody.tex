...
intervention while leaving the common cause intact, and hidden
...
criterion. The concession is worth making directly rather than
resisting. Closure is not a rival to screening off, and the argument
...
A defender of screening off who grants the modal reading has thereby
granted the criterion rather than refuted it, which is why the argument
is best understood as an explication of rainforest admission from
within rather than an objection to it.
...
downstream of that result rather than in tension with it, and it takes
their caveat as its point of departure rather than as a technical
footnote. It treats those observational diagnostics as strong evidence
...
lapses at exactly the point where parasitism becomes possible. The common-cause case developed below is the canonical wedge; the
...
variable that aggregates two subsystems driven by a shared upstream
process. The structure that matters is that the driver process is a
common cause of the proxy's present state and of its future state, so
that the apparent dependence of the macro-variable on its own past is
confounded by upstream structure the partition does not track. Under
...
more demanding than it may first appear, and it is worth stating
precisely, because a weaker reading would make the case below too easy.
Novelty requires \emph{distinct} macrodependencies: a binary star
...
Consider two driver processes \(U^1\) and \(U^2\), independent of each
other, each a symmetric two-state Markov chain on \(\{0,1\}\) that
flips with probability \(q\), where \(0 < q < 1/2\). Let \(A\) and \(B\)
be subsystems that track their respective drivers without lag, so that
\(A_{t+1} = U^1_{t+1}\) and \(B_{t+1} = U^2_{t+1}\). The micro-state is
\(X_t = (U^1_t, U^2_t, A_t, B_t)\). The candidate macro-variable is the
agreement of the two subsystems, \(Z_t = A_t \oplus B_t\), which takes
the value \(0\) when they agree and \(1\) when they disagree. In the
observed process the subsystems are synchronized with their drivers, so
\(A_t = U^1_t\), \(B_t = U^2_t\), and \(Z_t = U^1_t \oplus U^2_t\).

Write \(r = q^2 + (1-q)^2\) for the probability that two drivers
currently in agreement still agree at the next step, and note that the
probability that two currently disagreeing drivers come to agree is
\(2q(1-q) = 1 - r\). The candidate satisfies the whole package.

\begin{enumerate}
	\item
	      Unconditional relevance. Given the micro-state, the probability
	      that \(Z_{t+1} = 0\) is \(r\) when the drivers agree and
	      \(1 - r\) when they disagree. Since \(Z\) is a symmetric chain
	      its stationary probability is \(1/2\), and
	      \(r - 1/2 = 2(q - 1/2)^2 > 0\) for \(q \neq 1/2\). So
	      \(P(A \mid LLD) > P(A)\) and micro-detail is relevant.
	\item
	      Conditional irrelevance, in the strict rather than the
	      approximate form. When \(Z_t = 0\), the only micro-states the
	      observed process ever visits are those with
	      \(U^1_t = U^2_t\), and both of them, \((0,0)\) and \((1,1)\),
	      yield \(P(Z_{t+1} = 0) = r\). When \(Z_t = 1\), the only
	      visited realizations are those with \(U^1_t \neq U^2_t\), and
	      both yield \(1 - r\). Hence
	      ...
	      condition.
	\item
	      Novelty. \(Z\) is a symmetric two-state chain with persistence
	      \(r\), while each driver has persistence \(1 - q\). These differ
	      whenever \(q \neq 1/2\), since \(r - (1-q) = q(2q - 1)\). The
	      macro-law is therefore not the driver's law relabeled. And
	      \(Z\) cross-classifies: the macrostate \(Z = 0\) lumps
	      \((0,0)\) with \((1,1)\), cutting across the partition induced
	      by either driver's state. This is novelty in their sense, and
	      the case is not the binary star.
\end{enumerate}

So the proxy is admitted to the rainforest. Now consider the
micro-realizations of \(Z_t = 0\) that the observed process never
visits: those in which the subsystems agree while their drivers do not.
Suppose the system is in the observed state \(U^1_t = 0\),
\(U^2_t = 1\), \(A_t = 0\), \(B_t = 1\), so that \(Z_t = 1\), and apply
an admissible intervention that sets \(B_t = 0\), leaving both drivers
untouched. The system now occupies a realization of \(Z_t = 0\). From
there, \(P(Z_{t+1} = 0) = 1 - r\), because the drivers still disagree,
whereas every observed realization of \(Z_t = 0\) yields \(r\). Two
micro-realizations of the same macrostate have different macro-futures,
and \(r \neq 1 - r\) for \(q \neq 1/2\).

The intervention is minimal in the ways that matter. It targets the
candidate partition directly rather than the common cause, which is
left intact, so the case cannot be answered by saying that the
perturbation simply changed the upstream system. It is
regime-preserving: the laws are untouched, and at the next step the
subsystems resynchronize with their drivers, so the perturbation is a
one-step transient rather than a move to a different dynamical regime.
And it is independently admissible, since setting a subsystem's state is
a physically available operation in any regime in which the subsystems
are separately manipulable at all.

The diagnosis is exact. The proxy's clean macro-law holds only because,
...
\begin{enumerate}
	\item
	      ...
	      dynamics because the driver process keeps the subsystems pinned
	      to it.
	\item
...
\begin{enumerate}
	...
	\item
	      ...
	      distribution in which \(X_t\) is set by an admissible
	      intervention ranging over the reachable realizations of the
	      macrostate. This is the criterion's quantity. It vanishes for
	      every admissible intervention distribution just in case
	      within-class micro-differences do not alter the macro-transition
	      profile, which is transition closure as stated in
	      \S\ref{formal-statement}.
\end{enumerate}
...
why the literature has not needed to separate them. They come apart
...
leaving it as a parallel formalism. Strong lumpability is the limiting
case in which admissible leakiness vanishes for every distribution
supported on a macroclass, since it requires equal onward
macro-transitions for every microstate in the class rather than only for
those the observed distribution happens to weight. Weak lumpability, by
contrast, is what observational leakiness can certify: the macro-process
is Markov for the distribution at hand. The proxy is weakly lumpable and
not strongly lumpable, and the two leakiness quantities separate on it
for that reason.
