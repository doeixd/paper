...
The contrast with the monetary case also fixes the shape of the three
verdicts. Robust standing goes to candidates whose macro-transitions
survive admissible intervention across heterogeneous realizers.
Qualified standing goes to regime-fragile candidates, such as organisms,
whose screening structures hold across bounded horizons but degrade at
pathological or ecological boundaries, which is why closure can be
regime-robust without being regime-universal. Downgrade goes to
candidates whose apparent stability is borrowed, such as the proxies of
\S\ref{what-the-formal-machinery-alone-does-not-provide}, or whose
transition autonomy collapses once the supporting structure is
perturbed, even where the representation persists.
...
The common-cause proxy of
\S\ref{what-the-formal-machinery-alone-does-not-provide} is the exact
instance of this contrast, and it is worth reading in that light. There
the proxy and a transition-autonomous partition are not merely close on
observational fit: the proxy satisfies strict conditional irrelevance,
so no observational diagnostic separates them at all, and the verdicts
diverge only once the admissible intervention class is applied. A
near-tie in prediction is the general case; identity in prediction with
divergence in closure is the limiting one.

The general pattern is familiar once stated. A partition calibrated to
regularities that hold within a regime can match a
transition-autonomous rival while the regime holds, and come apart from
it under perturbations that redistribute those regularities without
respecting the partition's boundaries. What separates the two is not
predictive accuracy but whether the partition's own transition
structure survives the perturbation or requires renewed within-class
bookkeeping to be restored. That is the repair signature the criterion
picks out, and it is why observational fit cannot settle admission.
