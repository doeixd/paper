...
too permissive. A coding strategy can be genuinely general and
...
convenience into genuine ontological contact \citep{haugeland1998},
...
but left implicit the dynamical condition that separates genuine
higher-level kinds from merely disjunctive aggregates. More recent
...
paper takes that composite critique seriously: what is missing is
not more compression, but an explicit test of whether
macro-descriptions carry autonomous transition structure.
...
intuitively disjunctive or aesthetically odd.
...
objecthood verdicts \citep{ladyman2007}. With Rosas et al., the relationship is tool-user rather than
commentary: their formal closure diagnostics supply measurement
machinery, while the present paper supplies the philosophical
...
The claim that closure is sufficient for macro-objecthood needs a
statement of what it covers. It covers the question of whether there is
...
The foundational answer is that the system's own dynamics do. A regime
...
genuine feature of the system's dynamics rather than one drawn to suit
...
is robustly closed under declared constraints, and should be withheld
...
line falls elsewhere, and the two ways an environment can support a
candidate show where.
...
the regime intact. So the question for any candidate is not whether it
depends on its environment, since everything does, but whether its own
macrostate carries the forecast or only stands in for an off-partition
variable that the observed process keeps aligned with it. An organism
...
pressure. The view does not treat higher-level descriptions as
...
anti-gerrymandering result. It is not a complete metaphysics of
