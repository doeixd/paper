...
The criterion is also not offered as a complete account of objecthood.
It answers one question: whether a candidate carries autonomous
transition structure at its grain. Several neighboring accounts answer
different questions about the same entities, and they are best read as
complements rather than rivals. Contextual emergence shares the
regime-relativity defended here, since it treats higher-level
descriptions as licensed by contingent contexts that lower-level
description does not itself supply \citep{bishopatmanspacher2006}.
Process ontology shares the view of objects as persistent organizations
of change rather than static bundles of matter
\citep{dupre2018,jiang2025}. Structurally oriented hylomorphism is
compatible to the extent that ``form'' is read as the organizing
...
generated in some physical domains rather than a general condition on
it. Functional and historical accounts of artifacts explain why
something is classified and individuated as a hammer or a chair
\citep{thomasson2007}, which is a different question from what
autonomous dynamical structure it realizes. These accounts can explain
how closed structure is realized, stabilized, classified, or
individuated. What closure contributes is the dynamical component that
any of them presupposes when they treat a higher-level entity as doing
something in its own right: if genuinely autonomous macro-structure is
present, that autonomy shows up in the transition structure.

For the same reason, the criterion is not an answer to the special
composition question of when some parts compose a whole
\citep{vaninwagen1990,lewis1986}. It asks when a coarse-graining
carries its own dynamics, and the lower-level constituents figure only
as what realizes that dynamics. This leaves the view compatible with
several positions on composition, including structuralist views on
which talk of objects is dispensable in favor of structure
\citep{french2014}. Someone who accepts all the transition facts the
criterion identifies but declines to call the resulting structures
``objects'' disagrees with the present account mainly about the word.
The claim defended here is about robust macro-level commitment, and
nothing in the argument turns on the label.
...
The words ``test'' and ``criterion'' can suggest that closure is
something investigators do. It is not. Whether a partition carries
autonomous transition structure under a given regime, horizon, and
intervention class is a fact about the system, and it holds or fails
whether or not anyone has found it. The protocol developed below is how
that fact is discovered and how claims about it are kept honest. So
when the paper requires that regime, horizon, and admissible class be
declared before scoring, the requirement protects the evidence. It does
not make the regime. A regime is significant because the world supports
autonomous transition structure there. Investigators can be wrong about
where such regimes lie, but their existence is not up to anyone. This
separation is made explicit in the verdict structure of
\S\ref{reporting-and-evidential-discipline}, where a candidate's closure
profile is distinguished from the evidential grade of our access to it.
...
in overlapping regimes, and each can earn standing in its own regime.
Ordinary kind terms need not track these structures one to one. A
conceptual illustration makes the point. Suppose a lump of wood and
metal formed by accident and, under the interventions relevant to
striking and pulling nails, behaves exactly as a manufactured hammer
does. In that regime it realizes the same transition structure as the
hammer. Whether it is a hammer in the artifact-theoretic sense, given
that nobody made it for that purpose, is a separate question that
ordinary language bundles with the first. The criterion answers only the
first.
...
further, and the crux can be stated first: a reading that no stated
...
on candidates the debate cares about. Screening off properly understood
is modal screening off, and modal screening off is transition autonomy:
there are not two properties here but one, described once in the
vocabulary of conditional independence and once in the vocabulary of
dynamics. So the concession does not make the criterion idle. The
contribution is the test and
...
The point is not to pretend that human interests disappear. It is to
ensure that they cannot silently protect a favored partition from
downgrade.
...
A more global version of the worry asks what selects the right contexts
at all, given that some sufficiently narrow regime can be found in which
almost any candidate looks stable. The answer depends on separating two
kinds of robustness that the protocol checks, because they play
different roles. Robustness across the admissible intervention class is
part of what closure is. The criterion quantifies over every
within-class realization an admissible intervention can reach
(\S\ref{leakiness-as-canonical-target}), so a candidate that holds up
under some admissible perturbations and not others is not closed under
that class. Robustness across nearby regimes, horizons, and modelling
choices plays a different role. It is evidence that the regime under
test is a real feature of the system's dynamics rather than one tuned to
a favored partition. The narrow-regime worry is answered by that second
kind of robustness together with the admissibility constraints: a regime
drawn so tightly that only one partition survives in it will usually
fail projectibility under small context shifts
(\S\ref{regime-dependence-and-projectibility}), and a regime whose
boundaries were chosen by looking at closure scores violates the
selection discipline. No independently privileged scientific task is
needed to fix the right context. The right regimes are those in which
the world supports autonomous transition structure, and the protocol is
built to find them without being able to invent them.
...
with it. That is why admissibility is declared before scoring
(\S\ref{admissibility-and-hierarchical-evaluation}).
...
The graded assessments the criterion issues should be read in the same
spirit, with one distinction kept in view. A candidate's closure profile
reports where it sits in the space of more and less stable closure
relations, and that is a claim about the candidate rather than about
the state of the evidence. The commitment verdict built on the profile
also answers to the evidence, so a downgrade can reflect either a
fragile candidate or thin access to it
...
\begin{enumerate}
	...
	\item
	      ...
	      The unvisited realizations do not disturb this verdict, and it
	      matters why. The realizations of \(Z_t = 0\) with \(V_t = 1\)
	      receive probability zero in the observed process, so the
	      conditional probability is undefined on them and the strict
	      condition holds over them only vacuously. The condition is not
	      violated by the heterogeneous realizations; it is silent about
	      them, and silent about exactly the realizations on which the
	      closure question turns. The proxy does not merely approximate
	      screening off, in the way a deviant microstate would degrade
	      it. It satisfies the strict condition on every realization for
	      which the condition says anything at all.
	\item
...
settles it is a commitment the test does not contain.
...
A related worry runs in the opposite direction. Organisms, artifacts,
and institutions all depend on their surroundings: on gravity, on a
temperature range, on nutrient flows, on supporting practices. If
dependence on structure outside the partition is parasitism, the wedge
seems to condemn nearly every special-science kind. It does not, because
parasitism in the present sense is narrower than dependence. Background
conditions that the regime holds fixed are part of the regime's
specification. They are not borrowed by the candidate; they are the
setting in which the candidate's closure is assessed, and a change in
them is a change of regime rather than a perturbation within it. The
drivers in the proxy are not background conditions of this kind: they
vary within the regime, and the macro-future turns on their current
state. The proxy is parasitic for that reason. An admissible intervention on
the candidate itself, the reset line, decouples it from the structure
that sustains its apparent autonomy while leaving the regime intact. The
question for any candidate is therefore not whether it depends on its
environment, since everything does, but whether some admissible
perturbation acting on the candidate reaches within-class realizations
that the environment's support had been hiding. An organism kept alive
by a stable environment is not thereby parasitic. It would be parasitic
only if an admissible perturbation acting on it showed that its apparent
macro-dynamics had tracked some external driver all along.
...
two aligned. Parasitism is then not a metaphor. It names a candidate
whose apparent transition structure is the closure of a different
...
applicability. Three points blunt this. First, qualified verdicts are
comparative results, not silence: they rank candidate partitions,
locate where leakiness lives, and say what evidence would upgrade the
verdict, which is more than a permissive criterion says anywhere.
Second, rival criteria do not perform better where closure verdicts
are hard; they issue robust verdicts there, and the wedge cases show
that some of those verdicts certify borrowed autonomy. A criterion
that withholds robust standing where the evidence cannot support it is
not weaker than one that grants it there. Third, decisiveness tracks
the maturity of a domain's control ecology rather than a boundary
fixed in advance: as intervention practice accumulates, the same
criterion delivers stronger verdicts, so its reach grows with
experimental practice instead of being confined to clean cases by
construction.
...
in line with the three claims distinguished in
\S\ref{introduction-closure-and-the-admittance-problem}. Under that programmatic reading, the
...
\S\ref{why-closure-carries-ontological-weight}; readers who
withhold those commitments can still accept the narrower
