...
which contexts count. It does not. The protocol checks two kinds of
robustness, and they do different jobs.
...
not closed under that class. Its failure is a fact about it, not a gap
in the evidence.

Robustness across nearby regimes, horizons, and modelling choices is
different. It is not part of closure in a single regime; a candidate
can be closed in one regime and leak in the next, as ice does across a
phase boundary. It is evidence that the regime under test is a genuine
...
selection discipline outright. So closure is a local test in one sense
and not in another. Its verdict is always relative to a regime, but the
regime has to be identified from the system's physical features and
...
means here. The question for any candidate is therefore not whether it
depends on its environment, since everything does, but whether its own
macrostate is doing the predictive work or only standing in for some
off-partition variable that the observed process keeps aligned with
it. An organism kept alive by a stable environment is not parasitic
...
Both worries also bring out what the countermodel does and does not
show. It establishes a possibility, that the stated
